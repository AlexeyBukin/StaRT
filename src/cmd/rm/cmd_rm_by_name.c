...
t_msg				cmd_rm_group_force(t_parser *parser, t_rt *rt, t_obj *group, int rm_force)
{
	if (group->type == OBJ_GROUP)
	{
		...
		return (safe_rm(parser, scn_remove_by_name_grp(rt->scene, group->name)));
	}
	ft_free(parser->name);
...
t_msg				cmd_rm_by_name(t_rt *rt, t_parser *parser, int rm_force)
{
	...
	if (scn_get_mat_by_name(rt->scene, parser->name))
	{
		...
		return (safe_rm(parser, scn_remove_by_name_mat(rt->scene, parser->name)));
	}
