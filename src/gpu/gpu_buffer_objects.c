...
int				gpu_buffer_object_link_material(t_gpu *gpu)
{
	...
	while (obj_i < gpu->info.obj_num)
	{
		...
		while (mat_i < gpu->info.mat_num && (int)gpu->mat_buf[mat_i].id != gpu->obj_buf[obj_i].mat_index)
		{
			ft_printf(", %d", (int)gpu->mat_buf[mat_i].id);
			mat_i++;
		}
		ft_printf("\n");
//		if (mat_i == gpu->info.mat_num)
//			return (rt_err("Cannot link material"));
...
int 			gpu_buffer_objects_fill_rec(t_gpu *gpu, t_obj *obj, t_tfm *global)
{
	...
	if (obj->type == OBJ_COPY)
		return (gpu_buffer_objects_fill_rec(gpu, obj->content.copy, &obj->transform));
	if (obj->type == OBJ_GROUP)
	{
		i = 0;
		while (i < obj->content.group.child_num)
		{
			if (gpu_buffer_objects_fill_rec(gpu, obj->content.group.children[i], &obj->transform))
...
int				gpu_buffer_object_container(t_gpu *gpu, t_obj *obj)
{
	...
	else if (gpu_obj->type == SHP_CYLINDER)
	{
		...
		gpu_obj->shape.cylinder.pos = vec3_plus(obj->transform.pos_global, tmp);
		gpu_obj->shape.cylinder.cap = vec3_minus(obj->transform.pos_global, tmp);
