...
static void		rgb_to_bw_cycle(char *array, t_txr *texture,
								int old_pix_size, int new_pix_size)
{
	...
	{
		array[0] =
			(old_array[0] * 0.3 + old_array[1] * 0.59 + old_array[2] * 0.11);
		array[1] = array[0];
		array += new_pix_size;
...
int				txr_rgb_to_bw(t_txr *texture)
{
	...
	if (texture->type != TXR_RGB_8)
		return (rt_err("rgba_to_rgb(): given texture is not TXR_RGB_8"));
	new_pix_size = 2;
	old_pix_size = 3;
	array = texture->content;
	rgb_to_bw_cycle(array, texture, old_pix_size, new_pix_size);
	texture->stride = texture->width * new_pix_size;
	texture->type = TXR_BW_8;
	return (0);
}

static void		rgb_to_rgba_cycle(char *array, t_txr *texture,
								int old_pix_size, int new_pix_size)
{
	size_t		i;
	char		*old_array;
...
int				txr_rgb_to_rgba(t_txr *texture)
{
	...
	rgb_to_rgba_cycle(array, texture, old_pix_size, new_pix_size);
	texture->stride = texture->width * new_pix_size;
